...
load 'D:/test/4.3用/data转mat数据/data2子文件2';
...
%% 7 step PW-filter滤除PPG干扰点算法过程%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
ed=mean(diff(d))*0.8;%定义相邻两个R波之间使用的PPG数据段的终点（即所以R波间隔的平均值*0.8，但因数据不同可能需要调整参数）
for i=1:1:l-1
...
%%7 step PW-filter%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% 查找BP峰值点中的干扰点%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
for i=1:1:length(d)-1%对存储有所有R波峰值点位置信息的数组进行循环，查找每相邻两个R波之间的BP峰值点点个数
    m=1;
    s=1;
    num_max=[];%用于存储两个R波峰值点之间的BP峰值点位置信息
    %% 循环查找当前两个R波峰值点之间的BP峰值点
    for j=1:1:length(max_BP)%对存储有所有BP峰值点位置信息的数组进行循环
    ...
    if l_m==1%判断当前两个R波峰值点之间是否有且仅有一个BP峰值点
...
% hold on;
% for i=1:1:length(d)
%     text(d(i),ecg(d(i))+0.2,num2str(i),'HorizontalAlignment','center','VerticalAlignment','middle');
% end
figure(2)
plot(t,p);
hold on;
...
bp=p_bp;
pwtt=pwtt1;
% save(['实验数据/实验数据3/mat/data2_7step改正/PWTT/pwtt1/pwtt24'],'pwtt') ;
% pwtt=pwtt2;
% save(['实验数据/实验数据3/mat/data2_7step改正/PWTT/pwtt2/pwtt24'],'pwtt') ;
% pwtt=pwtt3;
% save(['实验数据/实验数据3/mat/data2_7step改正/PWTT/pwtt3/pwtt24'],'pwtt') ;
% save(['实验数据/实验数据3/mat/data2_7step改正/BP/bp24'],'bp') ;
plot(t2,p_bp);
