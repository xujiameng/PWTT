...
%Description：
% 该程序功能为：从原始信号数据计算得出有效的PWTT与BP峰值点位置信息并进行拟合。
% 程序原理及流程：
%     Step1: 计算PWTT,并对PWTT数据与BP峰值点位置数据经过改进的7-step滤波法，BP异常点检索方法对数据中的异常点进行剔除;
%     Step2: 从原始信号数据获取有效点后根据需要 计算所运行数据的最优的拟合结果
%            或根据设定的函数计算对所运行数据的预测gap值，进而进行拟合。
% 程序运行结果：
%     程序目标结果：以PWTT作为因变量,BP作为自变量，对这两组数据进行拟合
%     结果说明：输出图片为取平均值后（根据需要，一定情况下不需经过取平均值的计算）的有效点分布，及PWTT与BP的拟合直线
...
%Outputs：
%	输出图片为取平均值后（根据需要，一定情况下不需经过取平均值的计算）的有效点分布，及PWTT与BP的拟合直线；
%   R为拟合直线的拟合优度；
%   len为经过异常点剔除后的有效点个数。
